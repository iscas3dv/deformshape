...
class PointCloud_wo_FreePoints(Dataset):
    def __init__(self, pointcloud_path, on_surface_points, instance_idx=None, expand=-1, max_points=-1):
        super().__init__()
        ...
        self.on_surface_points = on_surface_points
        self.max_points = max_points
        self.iteration = self.max_points // self.on_surface_points
    ...
    def __getitem__(self, idx):
        ...
        on_surface_sdf = np.zeros((self.on_surface_points, 1))  # on-surface = 0
        off_surface_sdf = -np.ones((off_surface_samples, 1))
        
        off_surface_normals = np.ones((off_surface_samples, 3)) * -1
        coord = np.vstack([on_surface_coords,off_surface_coords])
        sdf = np.vstack([on_surface_sdf,off_surface_sdf])
        normals = np.vstack([on_surface_normals,off_surface_normals])

        return {'coords': torch.from_numpy(coord).float(),
        'sdf': torch.from_numpy(sdf).float(),
        'normals': torch.from_numpy(normals).float(),
        'instance_idx':torch.Tensor([self.instance_idx]).squeeze().long()}

class PointCloudMulti(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        batch = [data[idx] for data in self.datalist]
        model_input={
            'coords':torch.stack([b['coords'] for b in batch],dim=0)
        }
        gt = {
            'normals':torch.stack([b['normals'] for b in batch],dim=0),

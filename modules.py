...
class FCBlock(MetaModule):
    ...
    def __init__(self, in_features, out_features, num_hidden_layers, hidden_features,
        ...
        nls_and_inits = {'sine30':(Sine(30), lambda m:sine_init(m,30), first_layer_sine_init,last_layer_sine_init),
                         ...
                         'leakyrelu':(nn.LeakyReLU(negative_slope=0.1,inplace=True), init_weights_leakyrelu, None,None),
...
class SingleBVPNet(MetaModule):
    ...
    def __init__(self, out_features=1, type='sine', in_features=2,
                 mode='mlp', hidden_features=256, num_hidden_layers=3, outermost_linear=True,first_initial=True,last_initial=True, **kwargs):
        super().__init__()
        self.mode = mode
        self.net = FCBlock(in_features=in_features, out_features=out_features, num_hidden_layers=num_hidden_layers,
                        hidden_features=hidden_features, outermost_linear=outermost_linear, nonlinearity=type,first_initial=first_initial,last_initial=last_initial)
        print(self)
...
class SDFBVPNet(SingleBVPNet):
    def forward(self, model_input, params=None):
        ...
        # various input processing methods for different applications
        output = self.net(coords_org, self.get_subdict(params, 'net')) + torch.norm(coords_org,dim=-1,keepdim=True)
        return {'model_in': coords_org, 'model_out': output}

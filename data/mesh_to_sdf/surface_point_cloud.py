...
class SurfacePointCloud:
    ...
    def get_random_surface_points(self, count, use_scans=True):
        if use_scans:
            ...
            return self.points[indices, :], self.normals[indices, :]#修改；加上法线返回值
        else:
    ...
    def sample_sdf_near_surface(self, number_of_points=500000, use_scans=True, sign_method='normal', normal_sample_count=11, min_size=0, return_gradients=False):
        ...
        surface_points,_ = self.get_random_surface_points(surface_sample_count, use_scans=use_scans)
    ...
    #把表面的点和空间中的点分开存
    def sample_sdf_near_surface_2(self, number_of_points=500000, use_scans=True, sign_method='normal',
                                normal_sample_count=11, min_size=0, return_gradients=False):
        query_points_near_surface = []
        query_points_in_sphere = []
        surface_sample_count = int(number_of_points * 25 / 50) // 2
        surface_points,_ = self.get_random_surface_points(surface_sample_count, use_scans=use_scans)
        query_points_near_surface.append(surface_points + np.random.normal(scale=0.0025, size=(surface_sample_count, 3)))
        query_points_near_surface.append(surface_points + np.random.normal(scale=0.00025, size=(surface_sample_count, 3)))

        unit_sphere_sample_count = number_of_points - surface_points.shape[0] * 2
        unit_sphere_points = sample_uniform_points_in_unit_sphere(unit_sphere_sample_count)
        query_points_in_sphere.append(unit_sphere_points)
        query_points_near_surface = np.concatenate(query_points_near_surface).astype(np.float32)
        query_points_in_sphere = np.concatenate(query_points_in_sphere).astype(np.float32)

        if sign_method == 'normal':
            sdf_near_surface = self.get_sdf_in_batches(query_points_near_surface, use_depth_buffer=False, sample_count=normal_sample_count,
                                          return_gradients=return_gradients)
            sdf_in_sphere = self.get_sdf_in_batches(query_points_in_sphere, use_depth_buffer=False, sample_count=normal_sample_count,
                                          return_gradients=return_gradients)
        elif sign_method == 'depth':
            ...
            sdf_in_sphere = self.get_sdf_in_batches(query_points_in_sphere, use_depth_buffer=True, return_gradients=return_gradients)
        else:
            raise ValueError('Unknown sign determination method: {:s}'.format(sign_method))
        if return_gradients:
            sdf_near_surface, gradients_near_surface = sdf_near_surface
            sdf_in_sphere, gradients_in_sphere = sdf_in_sphere
        ...
        '''
        if return_gradients:
            return query_points_near_surface, sdf_near_surface, gradients_near_surface, query_points_in_sphere,sdf_in_sphere,gradients_in_sphere
        else:
            return query_points_near_surface, sdf_near_surface, query_points_in_sphere, sdf_in_sphere
...
def create_from_scans(mesh, bounding_radius=1, scan_count=1000, scan_resolution=400, calculate_normals=True):
    scans = []
